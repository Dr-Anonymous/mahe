let localStream, remoteStream, pc1, pc2;
...
function start() {
  ...
  if ($('#screenShare').attr('share') == 'yes') {
		...
		} else return navigator.mediaDevices.getDisplayMedia({video: true}).then(gotStream).then(gotDevices).then(callFrom).catch(handleError);
  } else {
...
function gotRemoteStream(e) {
  ...
  remoteVideo.play().then(_ => {remoteStream = e.streams[0];})
			.catch(error => {});
}
...
function onIceCandidate(pc, event) {
	...
	console.log('Added code: ',event, event.candidate);
  /*getOtherPc(pc)
