	...
	if ($('#screenShare').attr('share') == 'yes') {
		if ($("#screenShareAudio").attr('checked')) {
			navigator.mediaDevices.getDisplayMedia({
				video: true
			}).then(clubAudio).then(callFrom).catch(handleError);
		...
		} else return navigator.mediaDevices.getDisplayMedia({
			video: true
		}).then(gotStream).then(gotDevices).then(callFrom).catch(handleError);
	} else {
